...
async def fetch_cities():
    ...
    async with httpx.AsyncClient() as client:
        for batch in chunk_list(CITIES, chunk_size):
            ...
            # Request temperature, weather code, wind speed AND precipitation.
            # NOTE: the legacy `current_weather=true` block only returns
            # temperature/windspeed/weathercode - it never includes
            # precipitation, so rainfall was always hardcoded to 0 before.
            # The newer `current=` parameter set exposes precipitation too,
            # and Open-Meteo does not let the two be combined reliably, so
            # we use `current=` exclusively (with renamed fields).
            url = (
            ...
            for city, weather in zip(batch, batch_data):
                ...
                wind_speed = cur.get("windspeed_10m", 0.0)
